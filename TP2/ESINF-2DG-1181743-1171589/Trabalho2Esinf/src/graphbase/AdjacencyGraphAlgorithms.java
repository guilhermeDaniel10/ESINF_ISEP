 ...
 * 
 */
public class AdjacencyGraphAlgorithms {

    
    /**
     * Performs breadth-first search of the graph starting at vertex.
    ...
    static <V,E> void DFS(AdjacencyMatrixGraph<V,E> graph, int index, LinkedList<V> verticesQueue, boolean visited[]) {
        V vertice = graph.vertices.get(index);
        verticesQueue.add(vertice);
        visited[index] = true;
        for (V vert: graph.directConnections(vertice)) {
                ...
                for (int k = 0; k < newGraph.numVertices; k++) {
                    if(i !=k && newGraph.edgeMatrix[i][k] != null){
                        for (int j = 0; j < newGraph.numVertices; j++) {
                            if(i!=j && newGraph.edgeMatrix[k][j] != null){
